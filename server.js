...
app.put("/api/skills/:id", upload.single("img"), (req, res) => {
  try {
    const skill = skills.find((s) => s._id === parseInt(req.params.id));

    if (!skill) {
      res.status(404).send({ error: "The skill you wanted to modify is not available" });
      return;
    }
    ...
    skill.lessons = Number(req.body.lessons);
    skill.instructor = req.body.instructor;
    skill.description = req.body.description;

    if (req.file) {
      skill.img_name = req.file.filename;
    }

    res.status(200).send(skill);
  } catch (err) {
    console.error("PUT /api/skills failed:", err);
    res.status(500).send({ error: "Server error updating skill." });
  }
});
...
app.delete("/api/skills/:id", (req, res) => {
  ...
  if (!skill) {
    res.status(404).send({ error: "The skill you wanted to delete is not available" });
    return;
  }
